...
SELECT SUM( DATEDIFF( YEAR, Dob, GETDATE() ) ) AS SUM_AGE
FROM Patients
WHERE PatientId IN(1,2);  --patient_id = 1 or patientid = 2
...
SELECT Condition, COUNT(*) AS PatientCount
FROM MedicalHistory 
GROUP BY Condition HAVING Condition = 'Hypertension';
...
FROM 
    Users INNER JOIN Notifications ON Notifications.UserId = Users.UserId;
...
FROM Users AS u
INNER JOIN Patients AS p ON u.UserId = p.PatientId;
